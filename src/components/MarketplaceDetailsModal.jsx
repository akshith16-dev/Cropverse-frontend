...
export default function MarketplaceDetailsModal({ crop, onClose, onOrderPlaced }) {
  ...
  async function placeOrder(event) {
    ...
    const available = Number(crop.quantity_kg || 0);
    const price = Number(crop.price_per_kg || crop.current_price || 0);
    if (!price) { setOrderMessage("A market price is required before placing this order."); return; }
    if (!requested || requested <= 0) { setOrderMessage("Enter a valid order quantity."); return; }
    if (requested > available) { setOrderMessage(`Only ${available} kg is available.`); return; }
    try {
      setOrdering(true);
      setOrderMessage("");
      await api.post("/orders/", { baby_crop_id: crop.id, quantity_kg: requested, price_per_kg: price, order_type: orderType });
      setOrderMessage("Order placed successfully. You can track it from My Orders.");
      onOrderPlaced?.();
    } catch (error) {
      setOrderMessage(error.response?.data?.message || error.response?.data?.detail || "Could not place order.");
    } finally {
      setOrdering(false);
    }
  }
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm" onMouseDown={onClose}><div role="dialog" aria-modal="true" aria-label="Marketplace crop details" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/10 bg-slate-900/95 p-5 shadow-2xl sm:p-7" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-emerald-400">Marketplace crop</p><h2 className="mt-1 text-2xl font-bold capitalize">🌾 {value(crop, ["crop_name", "name"], `${value(crop, ["growth_stage"])} crop`)}</h2></div><button onClick={onClose} aria-label="Close crop details" className="rounded-xl p-2 hover:bg-white/10"><X /></button></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{details.map(([label, content]) => <div key={label} className="rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-xs text-slate-400">{label}</p><p className="mt-1 break-words font-medium capitalize">{label === "Current price" && String(content).match(/^\d/) ? `₹${content}/kg` : content}</p></div>)}</div><div className="mt-4 rounded-xl border border-white/10 bg-white/5 p-4"><p className="text-sm text-slate-400">Notes</p><p className="mt-1 text-slate-200">{value(crop, ["notes", "description"], "No notes provided by the farmer.")}</p></div><form onSubmit={placeOrder} className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4"><p className="font-medium">Place order</p><div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto]"><input type="number" min="1" step="0.01" max={crop.quantity_kg || undefined} value={quantity} onChange={(event) => setQuantity(event.target.value)} placeholder="Quantity in kg" className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 outline-none focus:border-emerald-500"/><select value={orderType} onChange={(event) => setOrderType(event.target.value)} className="rounded-xl border border-white/10 bg-slate-950 px-4 py-3 outline-none"><option value="spot">Spot</option><option value="contract">Contract</option></select></div>{orderMessage && <p className="mt-3 text-sm text-emerald-100">{orderMessage}</p>}<button disabled={ordering} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-medium hover:bg-emerald-500 disabled:opacity-50"><ShoppingCart size={18}/>{ordering ? "Placing..." : "Place order"}</button></form>{contactOpen && <div className="mt-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4"><p className="font-medium">Contact farmer</p><p className="mt-1 text-sm text-slate-300">{farmerName}</p>{farmerPhone !== "Not available" && <p className="mt-1 text-sm text-slate-300">Phone: {farmerPhone}</p>}{farmerEmail !== "Not available" && <a className="mt-1 block text-sm text-emerald-300 hover:text-emerald-200" href={`mailto:${farmerEmail}`}>Email: {farmerEmail}</a>}</div>}<div className="mt-6 grid gap-3 sm:grid-cols-2"><button disabled={!hasFarmerContact} title={hasFarmerContact ? "" : "Farmer contact is not included in the marketplace response"} onClick={() => setContactOpen((open) => !open)} className="flex items-center justify-center gap-2 rounded-xl border border-white/10 px-4 py-3 hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"><Phone size={18}/>Contact</button><button onClick={toggleWishlist} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 ${isSaved ? "border-pink-400 bg-pink-500/10 text-pink-300" : "border-white/10 hover:bg-white/10"}`}><Heart size={18} fill={isSaved ? "currentColor" : "none"}/>{isSaved ? "Saved" : "Wishlist"}</button></div></div></div>;
}
